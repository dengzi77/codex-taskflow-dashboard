...
import { archiveCodexDesktopThread } from "./codex-app-task.mjs";
import {
  CloudProxyError,
  createCloudProxy,
...
const WORKTREE_CLEANUP_STATUSES = new Set(["done", "canceled"]);
...
function parseProjectAutomation(body) {
  ...
  if (!Number.isInteger(body.concurrencyLimit) || body.concurrencyLimit < 1 || body.concurrencyLimit > 5) {
    throw new ApiError(400, "INVALID_FIELD", "'concurrencyLimit' must be an integer from 1 to 5");
  }
  if (!isAutomationSandbox(body.sandbox)) {
    throw new ApiError(400, "INVALID_FIELD", "'sandbox' must be workspace-write or danger-full-access");
  }
...
function parseAiThreadCreate(body) {
  ...
  return {
    ...
    codexProjectId: body.codexProjectId === undefined
      ? undefined
      : validateProjectId(body.codexProjectId),
...
async function removeTaskWorktree(task, nextStatus) {
  const context = task?.developmentContext;
  if (
    WORKTREE_CLEANUP_STATUSES.has(task?.status)
    || !WORKTREE_CLEANUP_STATUSES.has(nextStatus)
    || context?.type !== "worktree"
    || !context.path
  ) return;
  await execFileAsync(
    "git",
    ["-C", context.path, "worktree", "remove", "--force", context.path],
    { timeout: 30_000, maxBuffer: 2 * 1024 * 1024 },
  );
}

async function archiveTaskThread(task, nextStatus) {
  if (
    WORKTREE_CLEANUP_STATUSES.has(task?.status)
    || !WORKTREE_CLEANUP_STATUSES.has(nextStatus)
    || !task?.threadId
  ) return;
  await archiveCodexDesktopThread({
    debugPort: process.env.CODEX_TASKBOARD_CODEX_DEBUG_PORT ?? "9229",
  ...
  const entries = await new Promise((resolve, reject) => {
    ...
    child.once("spawn", () => {
      send({
        ...
        method: "initialize",
        params: {
          clientInfo: { name: "codex-taskflow-dashboard", version: "1.0.0" },
...
export function createTaskboardServer(options = {}) {
  ...
  const automationScheduler = new AutomationScheduler({
    database,
    codexExecutable: resolved.codexExecutable,
    onTaskMoved: (task) => events.emit("task.moved", { task }),
  });
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (pathname === "/api/local/ai/catalog") {
        ...
        assertAllowedQuery(
          url.searchParams,
          new Set(["projectId", "codexProjectId"]),
          "GET /api/local/ai/catalog",
        );
        const projectId = validateProjectId(url.searchParams.get("projectId") ?? undefined);
        const codexProjectIdValue = url.searchParams.get("codexProjectId");
        const codexProjectId = codexProjectIdValue === null
          ? undefined
          : validateProjectId(codexProjectIdValue);
        return sendJson(response, 200, await aiChat.getCatalog(projectId, codexProjectId));
      }
      ...
      if (pathname === "/api/tasks") {
        ...
        if (request.method === "POST") {
          ...
          automationScheduler.notifyTaskReady(task);
      ...
      if (taskRoute) {
        ...
        if (!action && request.method === "PATCH") {
          ...
          await removeTaskWorktree(previousTask, task.status);
        ...
        if (action === "move" && request.method === "POST") {
          ...
          await removeTaskWorktree(previousTask, task.status);
          await archiveTaskThread(previousTask, task.status);
