...
function aiChatThreadFromRow(row) {
  return {
    ...
    origin: {
      ...
      ...(row.origin_codex_project_id
        ? { codexProjectId: row.origin_codex_project_id }
        : {}),
  ...
  #migrate() {
    this.database.exec(`
      ...
      CREATE TABLE IF NOT EXISTS project_automations (
        ...
        concurrency_limit INTEGER NOT NULL DEFAULT 1 CHECK (concurrency_limit BETWEEN 1 AND 5),
        sandbox TEXT NOT NULL DEFAULT 'workspace-write' CHECK (sandbox IN ('workspace-write', 'danger-full-access')),
        model TEXT NOT NULL,
        reasoning_effort TEXT NOT NULL,
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL
    ...
    if (!aiChatThreadColumns.some((column) => column.name === "origin_codex_project_id")) {
      this.database.exec(`
        ALTER TABLE ai_chat_threads
        ADD COLUMN origin_codex_project_id TEXT;
      `);
    }
    ...
    if (!automationColumns.some((column) => column.name === "concurrency_limit")) {
      this.database.exec(`
        ALTER TABLE project_automations
        ADD COLUMN concurrency_limit INTEGER NOT NULL DEFAULT 1
        CHECK (concurrency_limit BETWEEN 1 AND 5);
      `);
    }
    if (!automationColumns.some((column) => column.name === "sandbox")) {
      this.database.exec(`
        ALTER TABLE project_automations
        ADD COLUMN sandbox TEXT NOT NULL DEFAULT 'workspace-write'
        CHECK (sandbox IN ('workspace-write', 'danger-full-access'));
      `);
    }
  ...
  createAiChatThread(input) {
    ...
    this.database.prepare(`
      INSERT INTO ai_chat_threads (
        id, title, status,
        origin_project_id, origin_codex_project_id, origin_project_name, origin_workspace_path,
    ...
    `).run(
      ...
      input.origin.codexProjectId ?? input.origin.projectId,
  ...
  upsertProjectAutomation(input) {
    ...
    this.database.prepare(`
      INSERT INTO project_automations (
        ...
        enabled_by_user, quota_aware, interval_minutes, interval_seconds, concurrency_limit,
