...
export class AiChatService {
  ...
  async getCatalog(projectId, codexProjectId) {
  ...
  async createThread(input) {
    const [catalog, resolved] = await Promise.all([
      this.getCatalog(input.projectId, input.codexProjectId),
      resolveAiWorkspace(
        input.projectId,
        this.codexStatePath,
        this.database,
    ...
    return this.database.createAiChatThread({
      ...
      origin: {
        ...
        codexProjectId: input.codexProjectId ?? resolved.project.id,
  ...
  async startTurn(threadId, input) {
    ...
    const [catalog, resolved] = await Promise.all([
      this.getCatalog(thread.origin.projectId, thread.origin.codexProjectId),
      resolveAiWorkspace(
        thread.origin.projectId,
        this.codexStatePath,
        this.database,
