...
test("the stable name and generated prompt keep one task turn concise", () => {
  assert.equal(
  ...
  assert.match(prompt, /认领并处理一个 todo/);
  assert.match(prompt, /读取议题和评论/);
  assert.match(prompt, /最新 version/);
  assert.match(prompt, /in_review/);
  assert.equal(prompt.split("\n").length, 3);
  assert.ok(prompt.length < 300);

  const claimedPrompt = buildTaskboardAutomationPrompt({
    ...baseRequest,
    taskIdentifier: "PPT-7",
    taskId: "task-7",
  });
  assert.match(claimedPrompt, /处理已认领任务 PPT-7（task-7）/);
  assert.doesNotMatch(claimedPrompt, /认领并处理一个 todo/);
});
