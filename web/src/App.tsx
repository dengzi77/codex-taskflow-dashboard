...
import { PRIMARY_BOARD_STATUSES, StatusDock } from "./components/StatusDock";
...
type AutomationConcurrencyLimit = 1 | 2 | 3 | 4 | 5;
...
function readShowEmptyColumns(): boolean {
  return window.localStorage.getItem(SHOW_EMPTY_COLUMNS_KEY) !== "false";
}
...
function readProjectAutomations(): ProjectAutomations {
  try {
    ...
    for (const [projectId, record] of Object.entries(value)) {
      ...
      const concurrencyLimit = candidate.concurrencyLimit ?? 1;
      const sandbox = candidate.sandbox ?? "workspace-write";
      if (
        ...
        || !isAutomationConcurrencyLimit(concurrencyLimit)
        || !isAutomationSandbox(sandbox)
...
function isAutomationConcurrencyLimit(value: unknown): value is AutomationConcurrencyLimit {
...
export function App() {
    ...
    setProjectAutomations((current) => {
      if (
        ...
        || (
          ...
          && current[projectId]?.concurrencyLimit === record.concurrencyLimit
          && current[projectId]?.sandbox === record.sandbox
  ...
  const reconcileProjectAutomation = useCallback(async () => {
    ...
    try {
      ...
      writeProjectAutomation(selectedProjectId, {
        ...
        concurrencyLimit: config.concurrencyLimit as AutomationConcurrencyLimit,
  ...
  const visibleStatuses = useMemo(
    () => PRIMARY_BOARD_STATUSES.filter((status) => (
  ...
  return (
    <div className={`app-shell${embedded ? " embedded" : ""}`} style={appShellStyle}>
      ...
      <AiChat
        ...
        codexProjectId={selectedProjectId === "local"
          ? hostContext?.projectId ?? null
