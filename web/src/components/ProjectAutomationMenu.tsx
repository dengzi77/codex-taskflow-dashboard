...
type ConcurrencyLimit = 1 | 2 | 3 | 4 | 5;
    ...
    >
      ...
      <label className="project-automation-field">
        ...
        <select
          ...
          onChange={(event) => submitChange({
            ...draft,
            concurrencyLimit: Number(event.target.value) as ConcurrencyLimit,
          })}
        >
          {[1, 2, 3, 4, 5].map((count) => (
      ...
      <label className="project-automation-field">
        <span>执行权限</span>
        <select
          className={draft.sandbox === "danger-full-access" ? "is-danger" : undefined}
          value={draft.sandbox}
          disabled={disabled}
          onChange={(event) => submitChange({
            ...draft,
            sandbox: event.target.value as AutomationSandbox,
          })}
        >
          <option value="workspace-write">工作区权限</option>
          <option value="danger-full-access">宿主机完整权限</option>
        </select>
      </label>
      <label className="project-automation-field">
        <span>模型</span>
