...
export function AiChat({ available, projectId, codexProjectId, issueId }: AiChatProps) {
  ...
  const catalogProjectId = snapshot?.thread.origin.projectId
    ...
    ?? projectId;
  const catalogCodexProjectId = snapshot?.thread.origin.codexProjectId
    ?? selectedThreadSummary?.origin.codexProjectId
    ?? draftOrigin?.codexProjectId
    ?? codexProjectId
    ?? catalogProjectId;
  ...
  useEffect(() => {
    ...
    setCatalogLoadedProjectId(null);
    setCatalogError(null);
    void getAiChatCatalog(catalogProjectId, catalogCodexProjectId, controller.signal).then(
      (next) => {
  ...
  }, [available, catalogCodexProjectId, catalogProjectId]);
  ...
  useEffect(() => {
    if (
      ...
      || (
        ...
        && draftOrigin.codexProjectId === (codexProjectId ?? projectId)
        && draftOrigin.issueId === (issueId ?? null)
      )
    ) return;
    restorePersistedConversationFromDraft();
  }, [codexProjectId, draftOrigin?.codexProjectId, draftOrigin?.issueId, draftOrigin?.projectId, issueId, projectId]);

  function beginNewConversation() {
    const input = buildThreadCreateInput(projectId ?? "", issueId, codexProjectId);
    if (!input) {
    ...
    setDraftOrigin({
      ...
      codexProjectId: input.codexProjectId ?? input.projectId,
  ...
  async function createThreadForDraftOrigin(): Promise<AiChatThread | null> {
    const origin = draftOrigin ?? (
      projectId ? { projectId, codexProjectId: codexProjectId ?? projectId, issueId } : null
    );
    ...
    try {
      const targetCatalog = catalogLoadedProjectId === input.projectId && activeCatalog
        ? activeCatalog
        : await getAiChatCatalog(input.projectId, input.codexProjectId);
