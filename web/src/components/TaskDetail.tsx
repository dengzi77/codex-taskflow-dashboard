...
export function TaskDetail({
  ...
  const [moveToTodoPromptOpen, setMoveToTodoPromptOpen] = useState(false);
  ...
  async function saveDescription() {
    ...
    if (normalized === currentTask.description) return true;
    return (await saveTask({ description: normalized }, "description")) !== null;
  }
  ...
  async function moveCommentedTaskToTodo() {
    const saved = await saveTask({ status: "todo" }, "status");
    if (!saved) return;
    setMoveToTodoPromptOpen(false);
    onClose();
  }
  ...
  function beginEdit(comment: Comment) {
    ...
    setEditingBody(normalizeCommentBody(comment.body));
  ...
  return (
    <section className="issue-detail" aria-label={`${task.identifier} 议题详情`}>
          ...
          <div className="issue-detail-main">
            <article className="issue-editor" aria-label="议题内容">
              <div className="issue-editor-content">
                ...
                />
                {editingDescription ? (
                  <Suspense fallback={<div className="issue-description-editor-loading" aria-label="正在加载编辑器" />}>
                    <RichTextEditor
                      value={description}
                      ...
                      onSave={() => {
                        void saveDescription().then((saved) => {
                      ...
                      {editingId === comment.id ? (
                        ...
                        comment.body && (
                          <div className="comment-body">
                            <DescriptionDocument value={normalizeCommentBody(comment.body)} />
                          </div>
                        )
                      )}
      ...
      {moveToTodoPromptOpen && (
        <div className="delete-backdrop" role="presentation" onMouseDown={(event) => {
          if (event.target === event.currentTarget && savingProperty !== "status") {
            setMoveToTodoPromptOpen(false);
          }
        }}>
          <div
            ...
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="move-to-todo-title"
            onKeyDown={(event) => {
              if (savingProperty === "status") return;
              if (event.key === "Escape") {
                event.preventDefault();
                ...
                event.preventDefault();
                event.stopPropagation();
                void moveCommentedTaskToTodo();
              }
            }}
          >
            <h2 id="move-to-todo-title">移到待办？</h2>
            <p>评论已发布。是否将 {currentTask.identifier} 移到待办？</p>
            <div>
              <button
                className="button secondary"
                type="button"
                disabled={savingProperty === "status"}
                onClick={() => setMoveToTodoPromptOpen(false)}
              >
                暂不移动
              </button>
              <button
                ...
                autoFocus
                disabled={savingProperty === "status"}
                onClick={() => void moveCommentedTaskToTodo()}
              >
                {savingProperty === "status" ? "移动中…" : "移到待办"}
              </button>
            </div>
          </div>
        </div>
