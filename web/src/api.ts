...
export async function getAiChatCatalog(
  ...
  const codexProjectQuery = codexProjectId && codexProjectId !== projectId
    ? `&codexProjectId=${encodeURIComponent(codexProjectId)}`
    : "";
  return request<AiChatCatalog>(
    `/api/local/ai/catalog?projectId=${encodeURIComponent(projectId)}${codexProjectQuery}`,
    { signal },
  );
}
