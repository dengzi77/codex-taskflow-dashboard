...
const installedDataDirectory = path.basename(path.dirname(projectRoot)) === "releases"
...
function startTaskboard({ detached, codexDebugPort }) {
  return spawn(process.execPath, [path.join(projectRoot, "scripts", "update-supervisor.mjs")], {
    cwd: projectRoot,
    detached,
    env: {
      ...process.env,
      CODEX_TASKBOARD_DATA_DIR: taskboardDataDirectory,
...
async function restartResidentInjectorForRefresh(port) {
  ...
  const restarted = await restartResidentInjector(port, {
  ...
  if (restarted.restarted) return restarted;

  const startupToken = randomUUID();
  const started = await startResidentInjectorAfterManagedRestart(port, startupToken);
  await waitForResidentInjectorReady(
    port,
    started.pid,
...
function frameUrlMatches(actualUrl, expectedUrl) {
  try {
...
async function waitForFrame(cdp, expectedUrl, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    if (
      targetInfos.some(
        (target) => target.type === "iframe" && frameUrlMatches(target.url, expectedUrl),
      ) ||
