...
async function execute(parsed, overrides) {
  ...
  switch (command) {
    ...
    case "issue get":
      expectOperandCount(parsed, 1);
      return getIssue(api, parsed.operands[0]);
    case "issue create":
      expectOperandCount(parsed, 0);
...
async function getIssue(api, taskId) {
  const result = await api.request("GET", taskPath(taskId));
  const { attachments } = await api.request("GET", `${taskPath(taskId)}/attachments`);
  return { ...result, task: { ...result.task, attachments } };
}
